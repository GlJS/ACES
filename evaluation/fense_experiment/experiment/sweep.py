...
sys.path.insert(0, os.path.join(os.getcwd(), "evaluation/fense_experiment/experiment"))
...
device_num = 1 if torch.cuda.is_available() else -1
...
class CalculateACESScore():
    ...
    def calculate_aces_score_for_single(self, cand, ref) -> Tuple[float]:
        ...
        for i, entity in enumerate(overlap):
...
def get_aces_score(cands: List[str], refs: Union[List[List[str]], List[str]],
                   pipe=None, fl_weighing=False, f1_weight=0.5,
    ...
    if isinstance(refs[0], list):
        ...
        refs_cas = pipe(refs_cas, batch_size=64)
...
if __name__ == "__main__":
    def main():
        wandb.init(project="aces-fl-sweep-base")
    ...
    sweep_configuration = {
        ...
        "metric": {
            "goal": "maximize",
            "name": "total_audiocaps"
        },
        "parameters": {
            "model": {"values": ["gijs/aces-roberta-base-13"]},
            "fl_weighing": {"values": [False, True]},
            "average_strategy": {"values": ["simple", "first", "max", "average"]},
            "use_sbert": {"values": [False, True]},
            "division": {"min": 0.0, "max": 1.0},
            "f1_calc": {"values": ["mean", "mean-max", "max-mean", "max-max"]},
            "f1_beta": {"min": 0.1, "max": 10.0}, 
            "f1": {"min": 0.1, "max": 10.0},
            "penalty_score": {"min": 1, "max": 1000},
            "f1_weight": {"min": 0.0, "max": 1.0},
            "apply_penalty": {"values": [False, True]},
            "overlap_type": {"values": ["cand", "ref", "both", "f1"]},
            "distance_technique": {"values": ["cosine", "euclidean"]},
            "use_score": {"values": ["pairwise", "mean", "no"]},
            "score_weighing": {"min": 0.0, "max": 1.0},
            "overall_sbert": {"values": [False, True]},
            "overall_sbert_weight": {"min": 0.0, "max": 1.0},
            "sbert_based_on_scores": {"values": [False, True]}
        }
    }

    # sweep_id = wandb.sweep(sweep_configuration, project="aces-fl-sweep-base")
    # wandb.agent(sweep_id, function=main)

    # get_results({
    #     "model": 'gijs/aces-roberta-13', 
    #     "division": 0.9985719914845788, 
    #     "fl_weighing": True, 
    #     'f1_beta': 15.296839666572074, #9
    #     "f1": 3.797569713186928,
    #     "average_strategy": "max", 
    #     "use_sbert": True,
    #     "f1_calc": "max-mean",
    #     "penalty_score": 1850, # 400
    #     "apply_penalty": True,
    #     "overlap_type": "both",
    #     "f1_weight": 0.9,
